...
> No more, no less.
>
> **Gestures and module paths below are as-shipped at v0.1.** Save is now
> `Cmd+S` / `:w` and push is `:gs`; the flat `firmware/src/*.rs` layout became
> `drivers/` + `infrastructure/` under
> [ADR-013](../adr.md#adr-013-firmware-architecture--adopt-the-hexagonal-ports-refactor-over-the-flat-crate).
> [`stack.md`](../reference/stack.md) has the current layout.
>
> Decisions referenced inline point at [`adr.md`](../adr.md). Tradeoff weights
> and the critical-performance budget live in [`qfd.md`](../quality/qfd.md). Project
...
7. **Spike 7 — git push.** Smoke test: from desktop-Rust first, then on
   ...
   hardware ([postmortem](../record/postmortems/2026-07-05-spike7-gix-https-push.md)).
...
in [qfd-house-1.md §3](../quality/qfd-house-1.md#3-house-of-quality--whats--hows).
...
Implemented in [`firmware::infrastructure::storage_sd`](../firmware/src/infrastructure/storage_sd.rs)
...
- **Off by default.** The radio is powered down (`esp_wifi_stop`) except
  ...
  [qfd-budget.md H13](../quality/qfd.md#6-critical-performance-budget).
...
[spike 7 postmortem](../record/postmortems/2026-07-05-spike7-gix-https-push.md).
PSRAM heap during push is a top-3 watched metric — see
[qfd-budget.md §6](../quality/qfd.md#6-critical-performance-budget).
...
[qfd-tradeoffs.md "Conflicts left explicitly unresolved by v0.1"](../quality/qfd.md#7-tradeoffs-and-their-why-linked-to-adrs).

| Risk                                                                                                                                          | Symptom we'd see                      | Fallback                                                                                                                                                                                      |
| --------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~`gix` smart-HTTP push doesn't work on `esp-idf-rs` mbedtls~~ **fired 2026-07-05**: gix has no HTTP(S) push at all (only `file://`/`ssh://`) | spike 7 (desktop)                     | **switched to `libgit2` (`git2`)** — mechanics proven on desktop; on-device libgit2/mbedtls cross-compile is the new gate. See [postmortem](../record/postmortems/2026-07-05-spike7-gix-https-push.md). |
