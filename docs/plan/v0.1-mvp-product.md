...
>
> **Gestures below are as-shipped at v0.1.** Save is now `Cmd+S` (or `:w`), push
> is `:gs` (or the bare `gs` in Normal mode) — `Ctrl-S` and `Ctrl-G` have no
> binding. See [ADR-010](../adr.md#adr-010-push-ux--atomic-ctrl-g-auto-timestamp-commit-message-no-user-prompt)
> and [`../CONTEXT.md`](../../CONTEXT.md#user-facing-actions) for the current model.
>
> Companion docs:
> [README](../../README.md) (overview) ·
> [technical design](v0.1-mvp-technical.md) (how it's built) ·
> [ADR log](../adr.md) (load-bearing decisions) ·
> [QFD](../quality/qfd.md) (requirements ↔ functions ↔ components) ·
...
[qfd-budget.md §6](../quality/qfd.md#6-critical-performance-budget); that's the
