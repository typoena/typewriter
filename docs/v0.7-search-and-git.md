...
- [x] `:gl` — pull: fetch + **fast-forward only**, refuse on divergence and
  ...
  - **Superseded (v0.9):** divergence is no longer refused — `:gl` now rebases
    ...
    `:gs` to push. The `diverged - resolve on a computer` snackbar is
    replaced by `rebased <oid> - :gs to push`. See
...
- [~] Close the push gap: 19.8 s at the kaizen close — the splice's 5.0 s
  ...
  run 1. **Warm clean push measured 2026-07-14 (run 4, as `:gs` after
  ...
  **splice depth × loose-write cost**, which is the same FAT-dir-scan
  ...
  future perf pass; a root-level file's warm `:gs` should sit ≈ 12–13 s.
