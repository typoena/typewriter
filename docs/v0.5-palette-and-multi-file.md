...
(palette + `:enew` replace it) and push is `:gs`.
...
prefix (`/sd/repo` → Tracked, `/sd/local` → Local); `:gs` is refused in-core in
...
the scoped file and, for a Tracked file, that it is local until `:gs`
(`deleted repo/notes.md - :gs to push`). Deferred to later: greying the
...
device 2026-07-12** — `:enew test.txt` → `:gs` → `:delete` → `:gs` removed
...
`.typoena.toml`, which rides the next `:gs` to other devices), and confirms
...
idle auto-save is **unformatted** — `:fmt` runs only on explicit `:w`/`:gs`, so
...
- [x] Delete a file — **core done (host-tested) 2026-07-12;** `:delete` unlinks
      ...
      the next `:gs` Push's staged set. **Spike 14 (on-device) found the
      ...
      now confirms the delete and flags that a Tracked file needs `:gs`.
      **Verified on device 2026-07-12** — the `:enew`→`:gs`→`:delete`→`:gs`
      cycle removed test.txt from origin.
- [~] Push is disabled / hidden when the current buffer is local-scope —
  **`:gs` / Push is blocked in-core for a Local buffer** (posts "Push
  unavailable (Local)"); the side-panel affordance that hides/greys the
  gesture is the remaining half.
- [ ] The side panel briefly shows file count on `:gs` when the push bundles
...
- [x] **Preferences file** `/sd/repo/.typoena.toml` — a git-tracked,
  ...
  - [x] `format_on_save` (bool, default `true`) — run `:fmt` (table alignment,
        ...
        is persisted, so `:gs` is **fmt → save → commit → push** and `:w`
        ...
        an explicit `:w`/`:gs`; the `save_on_idle` auto-save is deliberately
  ...
  - [ ] `auto_sync` (duration string, default `"10m"`; `"0"` / omitted
        ...
        **opportunistic, rate-limited** Push. Push when already awake + dirty
        ...
        `:gl`/`:gs` only). Marked `[~]`: parsed and preserved, no behaviour.
