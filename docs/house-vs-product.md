...
argued with evidence on both sides instead of resolved by fiat.
...
**What acting on the claim would change.** Candidate moves, deliberately
**not** taken on the day of the claim (re-deriving four houses on an
assertion minutes old would bake it in rather than test it):

- **Re-weight the flow cluster** — W7 and W13 up, possibly W2 — and
...
the claim quantitatively: **H1 type latency rose #5 → #2** and **C7, the
widget/editor layer where the palette and modal grammar live, rose
#5 → #2 past libgit2** — the derived ranking now points where the July
