...
impl Storage {
    ...
    fn log_load_failure(&self, path: &str, stage: &str, e: &std::io::Error, len: i64) {
        ...
        log::error!(
            "load_path FAILED path={path} stage={stage} errno={:?} kind={:?} len={len} probe_open={probe:?} volume={:?} heap: free={free} internal={internal} largest_internal={largest}",
            e.raw_os_error(),
            e.kind(),
            self.usage().map_err(|u| format!("{u:#}")),
        );
    }
