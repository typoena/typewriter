...
/// trustworthy. There is no battery-backed RTC, so the clock starts every power
/// cycle at the epoch and stays there until SNTP runs on the net thread — a year
/// before 2020 means "unset". Honours the timezone applied at boot.
