...
//! ([`keyboard_usb`], `hal::Keyboard`), Wi-Fi + SNTP bring-up ([`wifi_esp`]),
//! the BQ25896 charger and the power button ([`bq25896`], [`power_esp`]
//! `app::Power`), and the esp clock/system adapters ([`clock_esp`] `app::Clock`,
//! [`system_esp`] `app::System`). The layers above name only the traits (see the `hal` and
