...
[v0.10 power-path section](../docs/plan/v0.10-battery-and-sleep.md#power-switch--power-path--decided-2026-07-26).
...
| Battery                        | **EEMB 103395** — 1S LiPo 3.7 V 3700 mAh, 10.3 × 33 × 95 mm, JST-PH 2.0 plug ([Amazon](https://www.amazon.fr/dp/B08215B4KK)) | Installed, powering the bench build — measure the real cell (incl. lead exit) and fill the scad's `bat_*` `<< MEASURE >>`                                                                 |
| Power switch                   | **QitinDasen** Ø12 mm latching push button, red LED, IP66, pre-wired — bought ×3 (2026-08-01)                                | Gate signal only, never in the load path; flip on = boot, flip off = clean shutdown + sleeping-Typo off card; LED stays **unwired** — contract + rationale in the [v0.10 power-path section](../docs/plan/v0.10-battery-and-sleep.md#power-switch--power-path--decided-2026-07-26) |
...
[hardware/case/README.md](case/README.md).
...
| Threaded inserts              | **ruthex RX-6-32×3.8** brass heat-set, `GE-6-32X38-001` (bag of 50) | **×12**: bracket bosses ×4 and baseplate posts ×4 in the BODY, board standoffs ×4 in the BASEPLATE. Ø5.4 flange / Ø4.7 body; every bore is printed Ø4.8 with ≥1.8 mm of wall per the datasheet. Pressed in with a soldering iron before assembly — see *Fasteners* in [`hardware/case/README.md`](case/README.md) |
| Screws, insert joints         | **#6-32 × 7 mm thread**, flat pan head 1.0 × Ø6.5 ⟨qty 12⟩ | One screw for the whole machine — bracket ×4, baseplate ×4, mainboard ×4 (its corner holes are a Ø3.7 #6-32 clearance). Measured off the part, not a catalogue number — the model budgets bore depth at 8 mm of thread, so **caliper a screw if the batch changes**. The four baseplate heads sit in a drilled lamage in the plate's underside, flush with the desk; the other eight bear on a printed face |
