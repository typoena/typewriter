...
| Screws, PCBs                  | **#6-32**, same family as the insert joints ⟨length/qty ref?⟩ | Four corner holes, Ø3,7 mm clearance. The mainboard uses the case's screw family so one driver and one bag cover the whole build. **The standoffs must accept a #6-32**: self-tapping into a taller boss, since a heat-set insert needs 4.8 mm and the current posts are 3 mm |
