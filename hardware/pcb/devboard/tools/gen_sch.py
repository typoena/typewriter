#!/usr/bin/env python3
"""Génère le schéma de la devboard.
...
C("J11", "Connector_Generic:Conn_01x22", "DEVKITC-1 rangée gauche", HDR22,
  {"1": "+3V3", "2": "+3V3", "4": "EPD_BUSY", "5": "EPD_RST",
   ...
   "22": "GND"})
# RST (3) reste en l'air : la carte de développement a son bouton et son rappel,
# rien ici n'a de raison de la redémarrer.
J11_NC = ["3", "13", "14", "21"]
...
C("J12", "Connector_Generic:Conn_01x22", "DEVKITC-1 rangée droite", HDR22,
  {"1": "GND", "4": "IO1", "5": "IO2", "6": "IO42", "7": "KBD_5V_EN",
   ...
   "18": "PWR_SENSE", "19": "USB_DP", "20": "USB_DM", "21": "GND", "22": "GND"})
...
C("J6", "Connector:USB_C_Receptacle_USB2.0_16P", "USB-C CHARGE + PROG",
  ...
  {"A1": "GND", "A4": "VBUS", "A5": "CC1", "A9": "VBUS", "B1": "GND",
   "B4": "VBUS", "B5": "CC2", "B9": "VBUS", "A12": "GND", "B12": "GND", "SH": "GND"})
...
NC = {"J11": J11_NC, "J12": J12_NC, "J4": EPD_NC, "J6": J6_NC,
      "J7": ["A8", "B8"], "J8": ["10"]}
...
SHEETS = [
    ...
    ("mcu",     "02-mcu.kicad_sch",     "MCU - rangees ESP32-S3-DevKitC-1 (2 x 22, 22,86 mm), decouplage 3V3"),
    ("display", "03-display.kicad_sch", "Ecran GDEY0579T93 - etage de puissance du §9, FPC 24p, header DESPI de secours"),
    ("io",      "04-io.kicad_sch",      "IO - USB-C charge, USB-C clavier (hote), clavier interne JST-PH, microSD, extension"),
]
...
for name, fname, titre in SHEETS:
    ...
    out = HEADER.format(root=SHEET_UUIDS[name], paper="A2", title="Typoena devboard",
...
root = HEADER.format(root=ROOT, paper="A4", title="Typoena devboard",
