...
- **Tray mount.** The keyboard PCB screws to integral posts on the bay floor
  ...
  (`Hk = 26` at the front) hides the plate edge, caps sit proud; toward the
  ...
  **`Hk` and `kb_post_h` track the base model's `Hf`.** `Hf` isn't only the
  ...
  put: at the base model's +4 with these left alone, the caps drop from **3.1 mm
  proud of the wall to −0.9 mm** and the back row sits in a well. Both carry the
  same +4 (`Hk` 22→26, `kb_post_h` 8→12), which restores 3.1 mm over the wall and
