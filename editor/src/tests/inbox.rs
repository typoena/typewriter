...
fn inbox_creates_todays_note_prefilled_dirty_and_in_normal() {
    ...
    // `:inbox` reads the card's contents to decide new-vs-existing, so the
    // walk has to have reported — here, an empty card.
...
#[test]
fn resolving_a_held_inbox_persists_the_buffer_it_parks() {
    // Nobody pressed a key for this switch, and parking keeps the outgoing
    // buffer's text in RAM only — idle-save, `:w`, `:gs` and the panic scribe
    // all write the active buffer. Without a Save here, a paragraph typed while
    // the hold stood is one battery death from gone.
    let mut e = Editor::with_file("/sd/repo/notes.md".into(), Scope::Tracked, String::new());
    e.set_file_list(Vec::new());
    ex(&mut e, "inbox");
    assert!(e.inbox_pending(), "no date yet");
    let _ = e.take_effects();

    e.handle(Key::Char('i'));
    for c in "a paragraph".chars() {
        e.handle(Key::Char(c));
    }
    e.handle(Key::Escape);
    assert!(e.dirty());
    ...
    e.set_today(Some(TODAY));
    assert!(!e.inbox_pending(), "the date resolved it");
    assert!(
        e.take_effects().contains(&Effect::Save {
            path: "/sd/repo/notes.md".into(),
            scope: Scope::Tracked,
            contents: "a paragraph".into(),
        }),
        "the parked buffer's prose is written before the switch"
    );
}
