...
fn fuzzy_score_matches_subsequence_case_insensitively() {
    ...
    assert!(fuzzy_score("NOTES", "repo/notes.md").is_some());
    assert!(fuzzy_score("rpnm", "repo/notes.md").is_some());
    assert!(fuzzy_score("xyz", "repo/notes.md").is_none());
    assert_eq!(fuzzy_score("", "anything"), Some(0));
}

#[test]
fn fuzzy_score_space_matches_any_separator() {
    assert!(fuzzy_score("la conv", "repo/la-convergence.md").is_some());
    assert!(fuzzy_score("notes md", "repo/notes.md").is_some());
    assert!(fuzzy_score("repo notes", "repo/notes.md").is_some());
    assert!(fuzzy_score("la conv", "repo/laconvergence.md").is_none());
}
...
fn half_page_keys_jump_the_selection_clamping_at_the_ends() {
    ...
    assert_eq!(e.palette_sel, HALF_PAGE);
...
fn command_mode_stays_open_across_multiple_toggles() {
    ...
    e.handle(Key::Enter);
    assert_eq!(e.prefs().line_numbers, !before);
    assert_eq!(e.mode(), Mode::Palette);
    assert_eq!(kinds(&e.take_effects()), vec![Kind::SavePrefs]);
...
fn next_option_rotates_and_wraps() {
    ...
    assert_eq!(next_option("dark", &THEME_OPTIONS), "light");
    assert_eq!(next_option("10m", &AUTO_SYNC_OPTIONS), "15m");
    assert_eq!(next_option("30m", &AUTO_SYNC_OPTIONS), "2m");
}
...
fn next_usize_option_rotates_wraps_and_snaps() {
    ...
    assert_eq!(next_usize_option(3, &SCROLL_MARGIN_OPTIONS), 0);
    assert_eq!(next_usize_option(9, &SCROLL_MARGIN_OPTIONS), 0);
}
...
fn running_the_scroll_margin_command_walks_the_presets_and_persists() {
    ...
    e.handle(Key::Enter);
    assert_eq!(e.prefs().scroll_margin, 3);
    assert_eq!(e.mode(), Mode::Palette);
    assert_eq!(kinds(&e.take_effects()), vec![Kind::SavePrefs]);
    for expected in [0, 1] {
        e.handle(Key::Enter);
...
fn draw_in_command_mode_does_not_panic() {
    let mut e = palette_type(&["/sd/repo/notes.md"], ">");
    let _ = e.draw(true);
    let mut none = palette_type(&["/sd/repo/notes.md"], ">zzzzz");
...
fn new_file_step_prefills_the_active_buffers_folder() {
    ...
    e.handle(Key::Enter);
    assert_eq!(e.palette_query, "repo/journal/");
...
fn new_file_tab_with_no_matching_folder_is_a_noop() {
    ...
    e.handle(Key::Char('\t'));
    assert_eq!(e.palette_query, "repo/zzz");
}
