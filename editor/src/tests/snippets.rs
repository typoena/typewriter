...
fn strip_labels_reduces_placeholders_to_bare_stops() {
    ...
    assert_eq!(strip_stop_labels("[$1]($2)$0"), "[$1]($2)$0");
    assert_eq!(strip_stop_labels("price: $ and ${3:x}"), "price: $ and $3");
}
...
fn parse_snippets_empty_and_malformed() {
    ...
    assert!(Snippets::parse("{ not json").is_err());
}
...
fn tab_expands_prefix_and_lands_on_first_stop() {
    ...
    assert_eq!(e.snippet_stops, vec![3, 4]);
}
...
fn tab_advances_stops_and_typing_shifts_pending_ones() {
    ...
    assert_eq!(e.snippet_stops, vec![6, 7]);
    e.handle(Key::Char('\t'));
    assert_eq!(e.caret, 6);
